...
export function AdminUsers({ currentUserId, onClose }: { currentUserId: string; onClose: () => void }) {
  ...
  const invite = async (event: React.FormEvent) => {
    ...
    const { data, error } = await supabase.functions.invoke<{
      emailMode?: 'temporary-password' | 'password-reset';
      temporaryPassword?: string;
    }>('admin-create-user', { body: { email, fullName } });
    if (error) setMessage({ type: 'error', text: cloudErrorMessage(error) });
    else {
      const delivery = data?.emailMode === 'password-reset'
        ? `Email di cambio password inviata a ${email}. Password provvisoria di emergenza: ${data.temporaryPassword}`
        : `La password provvisoria è stata inviata a ${email}.`;
      setMessage({ type: 'success', text: `Utente creato. ${delivery}` });
